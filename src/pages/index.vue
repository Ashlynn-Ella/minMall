              ...
              <div class="children">
                <ul v-for="(item, index) in productList" :key="index">
                  <li class="phone-item" v-for="sub in item" :key="sub.id">
                    <a
                      :href="
                        sub.id ? '/#/products/' + sub.id : '/#/products/45'
                      "
                    >
        ...
        <div class="product-info">
          ...
          <ul class="info-right">
            <li v-for="(item, index) in menuList" :key="index">
              <span :class="index % 2 ? 'news' : 'seckill'">{{
                index % 2 ? "新品" : "秒杀"
              }}</span>
              <a href="javascript:;">
                <div class="item-img">
...
export default {
  ...
  setup() {
    ...
    const getProductList =async ()=>{
      ...
      menuList.value = res.list.slice(6, 14);
      productList.value = [res.list.slice(0,4),res.list.slice(4,8)]
      // for (let i = 0; i < 6; i++) {
      //   let key = (i + 1) * 4;
      //   productList.value[i] = res.list.slice(i * 4, key);
      // }
    ...
    const addCart =async (id)=> {
      const res = await axios.post("/carts", {
       ...
       store.dispatch("getCartCount", res.cartTotalQuantity)
    }
    const goToCart = ()=> {
      router.push("/cart");
    }
